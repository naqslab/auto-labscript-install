...
def update_labscript_ini(app_name, updates):
    ...
    print(f"[prepare-configs] - Updating {ini_path}")
    ...
    print(f"[prepare-configs] - Updated settings in {app_name}.ini successfully")


example_experiment_file = os.path.join(
    labscriptlib_dir, "example_experiment.py"
)
example_analysis_script = os.path.abspath(
    os.path.join("example_files", "example_analysis_script.py")
)  # doesn't come with labscript

if sys.platform == "win32":
    example_experiment_file = example_experiment_file.replace(
        "\\", "\\\\"
    )  # double backslashes in the string
    example_analysis_script = example_analysis_script.replace(
        "\\", "\\\\"
    )  # double backslashes in the string

# The configs like to have the path value as a string like 'path/to/folder-or-script/'
current_labscript_file_value = f"'{example_experiment_file}'"
singleshot_value = f"[('{example_analysis_script}', 1)]" # 0/1 corresponds to lyse gui ticking 
...
# blacs has .h5 config instead of .ini
...
with h5py.File(h5file_path, "r+") as hdf5_file:
    ...
    attrs = ast.literal_eval(server_attrs)

    # print(attrs, type(attrs))

    attrs["server"] = "127.0.0.1"
    attrs["send_to_server"] = False
    dataset.attrs["analysis_data"] = str(attrs)
